...
1. 首先我们在`source/_posts`目录下建一个专门用于存放图片的文件夹`images`，在 markdown 中我们使用`![name](picture.png)`来插入图片,这样我们在 vscode 中就可以预览图片了。
2. 编写一个插件`hexo-li-assert-image`，用于将`![name](picture.png)`渲染后的`<img>`中的 src 替换为`/images/picture.png`

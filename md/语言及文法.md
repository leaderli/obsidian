...
右线性（Right Linear）文法：$A\to \omega$ 或 $A \to \omega B$
左线性（Left Linear）文法：$A \to B \omega$ 或 $A \to \omega$
...
```ad-summary
title:右线性文法
1. $S \to a\mid b\mid c\mid d$
2. $S \to aT\mid  bT\mid cT\mid dT$
1. $T \to a\mid b\mid c\mid d\mid 0\mid 1\mid 2\mid 3$
2. $T \to aT\mid  bT\mid cT\mid dT\mid 0T \mid 1T\mid 2T \mid 3T$
```

...
我们通过分析反射的API[[#newInstance|源码]] ，最终都会指向`ReflectionData`，当我们找需要的属性或方法时，都会拷贝一份出来，从而实现数据的隔离。
...
执行类acquireMethodAccessor的初始化过程
...
根据class，和方法的标识来使用ASM技术生成一个可被invoke0直接使用的字节码
